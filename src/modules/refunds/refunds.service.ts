...
import { BookingStatus, RefundStatus } from '../../common/enums/booking-status.enum';
...
export class RefundsService {
    ...
    async requestRefund(userId: string, bookingId: string, reason: string) {
        ...
        {
            throw new BadRequestException('Refund can only be requested for cancelled bookings');
        }
    ...
    async adminApproveRefund(adminId: string, bookingId: string, refundAmount: number, refundAdminNote: string) {
        ...
        {
            throw new BadRequestException('Refund amount cannot specify more than the paid amount');
        }
    ...
    async markRefundProcessed(adminId: string, bookingId: string) {
        ...
        {
            throw new BadRequestException('Refund must be APPROVED before it can be marked as PROCESSED');
        }
        ...
        await this.transactionModel.updateMany(
            { booking: booking._id as any, type: TransactionType.REFUND, status: TransactionStatus.PENDING } as any,
            { $set: { status: TransactionStatus.SUCCESS, processedAt: new Date(), processedBy: new Types.ObjectId(adminId) } }
        );
    ...
    async getRefundRequests(query: any) {
        ...
        {
            filter.refundStatus = { $in: [RefundStatus.REQUESTED, RefundStatus.APPROVED, RefundStatus.REJECTED, RefundStatus.PROCESSED] };
        }
