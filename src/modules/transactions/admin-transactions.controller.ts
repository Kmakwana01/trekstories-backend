import { Controller, Get, Post, Patch, Body, Param, UseGuards, Query, Res, NotFoundException } from '@nestjs/common';
...
export class AdminTransactionsController {
    ...
    @Patch(':id/approve')
    async approveReceipt(@Param('id') id: string, @CurrentUser('_id') adminId: string) {
        return this.transactionsService.approvePayment(id, adminId);
    }

    @Patch(':id/reject')
    async rejectReceipt(@Param('id') id: string, @Body('reason') reason: string, @CurrentUser('_id') adminId: string) {
