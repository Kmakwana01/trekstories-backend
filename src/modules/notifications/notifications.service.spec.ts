...
describe('NotificationsService', () => {
    ...
    it('should create a notification', async () => {
        const result = await service.createNotification('user123', 'general', 'Title', 'Msg');
        expect(result).toBeDefined();
        expect(result.title).toBe('Title');
    });

    it('should get notifications', async () => {
        const result = await service.getNotifications('user123', { page: 1, limit: 10 });
    ...
    it('should send email to queue', async () => {
        await service.sendEmail('test@test.com', 'Subject', 'otp', { otp: '123456' });
