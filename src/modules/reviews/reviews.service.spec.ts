...
describe('ReviewsService', () => {
    ...
    describe('create', () => {
        ...
        it('should throw BadRequestException if not own booking', async () => {
            bookingModel.findById.mockResolvedValue({ ...mockBooking, user: 'other' });
            const dto = { bookingId: 'booking123', rating: 5, comment: 'Great!' };
            await expect(service.create('user123', dto)).rejects.toThrow(BadRequestException);
        });
        ...
        it('should throw ConflictException if already reviewed', async () => {
            ...
            const dto = { bookingId: 'booking123', rating: 5, comment: 'Great!' };
            await expect(service.create('user123', dto)).rejects.toThrow(ConflictException);
        });
    });

    describe('approve', () => {
        it('should approve review and update tour rating', async () => {
            const reviewInstance = { ...mockReview, save: jest.fn().mockResolvedValue(this) };
            reviewModel.findById.mockResolvedValue(reviewInstance);
            reviewModel.aggregate.mockResolvedValue([{ _id: 'tour123', avgRating: 4.5, nRating: 1 }]);
    ...
    describe('delete', () => {
        it('should delete review and update rating if it was approved', async () => {
            reviewModel.findById.mockResolvedValue({ ...mockReview, status: 'approved' });
