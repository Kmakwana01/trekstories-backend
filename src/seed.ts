...
async function bootstrap() {
    ...
    // 4. Create Tour Dates
    const datePatterns = [
        { startOffset: -40, status: TourDateStatus.COMPLETED, totalSeats: 20, bookedSeats: 20 },
        { startOffset: -10, status: TourDateStatus.CANCELLED, totalSeats: 18, bookedSeats: 0 },
        { startOffset: 15, status: TourDateStatus.UPCOMING, totalSeats: 20, bookedSeats: 14 },
        { startOffset: 30, status: TourDateStatus.UPCOMING, totalSeats: 25, bookedSeats: 3 },
        { startOffset: 60, status: TourDateStatus.UPCOMING, totalSeats: 25, bookedSeats: 0 },
        { startOffset: 120, status: TourDateStatus.UPCOMING, totalSeats: 30, bookedSeats: 0 },
    ];

    const daysFromNow = (n: number): Date => { const d = new Date(); d.setDate(d.getDate() + n); return d; };
    const addDays = (date: Date, n: number): Date => new Date(date.getTime() + n * 86_400_000);
    ...
    const date1: any = seededTourDates.find(d => d.tour.toString() === tour1._id.toString() && d.status === TourDateStatus.UPCOMING);
    ...
    const b1: any = await bookingModel.create({
        ...
        travelers: [
            { fullName: 'John Doe', age: 39, gender: Gender.MALE, phone: c1.phone, idNumber: 'AD100' },
            { fullName: 'Jane Doe', age: 36, gender: Gender.FEMALE, phone: c1.phone, idNumber: 'AD101' },
        ],
        totalTravelers: 2,
    ...
    const date2: any = seededTourDates.find(d => d.tour.toString() === tour2._id.toString() && d.status === TourDateStatus.COMPLETED);
    ...
    const b2: any = await bookingModel.create({
        ...
        travelers: [
            { fullName: 'Priya Sharma', age: 32, gender: Gender.FEMALE, phone: c2.phone, idNumber: 'PR221' },
        ],
        totalTravelers: 1,
        baseAmount: 22000,
        discountAmount: 0,
