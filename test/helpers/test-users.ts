...
export async function getUnifiedTokens(app: INestApplication) {
    ...
    // Setup Admin
    let adminToken = await loginOrRegister(app, E2E_ADMIN, Role.ADMIN, userModel);

    // Setup Customer
    let customerToken = await loginOrRegister(app, E2E_CUSTOMER, Role.CUSTOMER, userModel);

    return { adminToken, customerToken };
}

async function loginOrRegister(app: INestApplication, creds: any, role: Role, userModel: any) {
    // 1. Try login
    ...
    {
        throw new Error(`Unified user registration failed for ${creds.email}: ${JSON.stringify(regRes.body)}`);
    }

    // 3. Verify OTP and promote if admin
    const user = await userModel.findOne({ email: creds.email });
    if (!user) throw new Error(`User not found after registration: ${creds.email}`);
